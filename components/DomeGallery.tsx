...
const DEFAULT_IMAGES: ImageItem[] = [
  {
    ...
    alt: "Doctor with patient in wheelchair",
  },
  {
...
export default function DomeGallery({
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      ...
      prev = now;
      // Pause while dragging, during inertia, or when an image is enlarged
      if (
        !draggingRef.current &&
        !inertiaRAF.current &&
        ...
        const nextY = wrapAngleSigned(
          rotationRef.current.y + autoRotateSpeed * dt * 60
        );
      ...
      onDrag: ({
        ...
        if (last) {
          ...
          if (
            ...
            vx2 = clamp((mx / dragSensitivity) * 0.02, -1.2, 1.2);
            vy2 = clamp((my / dragSensitivity) * 0.02, -1.2, 1.2);
          }

          if (Math.abs(vx2) > 0.005 || Math.abs(vy2) > 0.005) {
            startInertia(vx2, vy2);
          }
